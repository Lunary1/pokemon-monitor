...
| Secret scanning | Add a pre-commit or CI check (gitleaks or GitHub's built-in secret scanning, free on public repos) once CI exists | Cheap insurance, catches accidental `.env` commits |
| Dependency hygiene | `npm audit` (or Dependabot alerts, free on GitHub) reviewed at each release prep phase; update on `high`/`critical` findings before release | Small surface area, but `got`/`cheerio`/`playwright` all touch untrusted external content |
| Dependency updates | Dependabot PRs for minor/patch auto-mergeable after CI green; major version bumps get a design note | Keeps drift low without manual toil |
